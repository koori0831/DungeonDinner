...
function saveEvent(event) {
 ...
 const opts = event.options.map(o => {
   ...
   if(o.tool) data.push(`<LockTooltip>k__BackingField: ${q('필요한 도구: '+toolNames[o.tool])}`,`<KeyItem>k__BackingField: ${ref(tools[o.tool])}`,'<IsUnLockOption>k__BackingField: 0',`<IsUseItemOption>k__BackingField: ${o.consume?1:0}`,'LogStatus: 3');
...
console.log(`Created equipment expansion: ${events.length} events, ${events.reduce((n,e)=>n+e.options.length,0)} choices, existing equipment and image prefabs reused.`);
