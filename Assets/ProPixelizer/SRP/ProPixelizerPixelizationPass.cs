...
namespace ProPixelizer
{
    ...
    public class PixelizationPass : ProPixelizerPass
    {
        ...
        {
#if UNITY_6000_0_OR_NEWER
            // RenderGraph records offset passes below AfterRenderingOpaques before the built-in opaque pass.
...
#if UNITY_6000_0_OR_NEWER
        private sealed class PixelMapPassData
        {
            internal TextureHandle Metadata;
            internal TextureHandle MetadataDepth;
            internal TextureHandle SceneDepth;
            internal Material Material;
            internal bool Orthographic;
            internal bool OverlayCamera;
        }

        private sealed class ApplyPixelizationPassData
        {
            internal TextureHandle OriginalScene;
            internal TextureHandle PixelizationMap;
            internal TextureHandle MetadataDepth;
            internal TextureHandle SceneDepth;
            internal Material Material;
            internal bool WriteDepth;
            internal GlobalKeyword DepthOutputKeyword;
        }

        private sealed class CopyDepthPassData
        {
            internal TextureHandle Source;
            internal Material Material;
        }

        private sealed class CopyColorPassData
        {
            internal TextureHandle Source;
        }
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            if (cameraData.isPreviewCamera || resourceData.isActiveTargetBackBuffer)
                return;
            ...
            var mapDescriptor = colorDescriptor;
            mapDescriptor.colorFormat = RenderTextureFormat.ARGB32;
            mapDescriptor.graphicsFormat = UnityEngine.Experimental.Rendering.GraphicsFormat.R8G8B8A8_UNorm;

            var depthDescriptor = cameraData.cameraTargetDescriptor;
            depthDescriptor.useMipMap = false;
            depthDescriptor.colorFormat = RenderTextureFormat.Depth;

            var originalScene = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, colorDescriptor, "ProP_OriginalScene", false, FilterMode.Point);
            var pixelizationMap = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, mapDescriptor, "ProP_PixelizationMap", false, FilterMode.Point);
            var pixelatedScene = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, colorDescriptor, "ProP_PixelatedScene", false, FilterMode.Point);
            var pixelatedDepth = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, depthDescriptor, "ProP_PixelatedScene_Depth", false, FilterMode.Point);

            var colorTarget = resourceData.activeColorTexture;
            var depthTarget = resourceData.activeDepthTexture;
            var metadata = OutlinePass.OutlineObjectTexture;
            var metadataDepth = OutlinePass.OutlineObjectDepthTexture;

            using (var builder = renderGraph.AddRasterRenderPass<CopyColorPassData>(
                "ProPixelizer Copy Original Scene", out var passData))
            {
                passData.Source = colorTarget;
                builder.UseTexture(colorTarget, AccessFlags.Read);
                builder.SetRenderAttachment(originalScene, 0, AccessFlags.Write);
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PixelMapPassData>(
                "ProPixelizer Pixelization Map", out var passData))
            {
                passData.Metadata = metadata;
                passData.MetadataDepth = metadataDepth;
                passData.SceneDepth = depthTarget;
                passData.Material = Materials.PixelizationMap;
                passData.Orthographic = cameraData.camera.orthographic;
                passData.OverlayCamera = cameraData.renderType == CameraRenderType.Overlay;

                builder.UseTexture(metadata, AccessFlags.Read);
                builder.UseTexture(metadataDepth, AccessFlags.Read);
                builder.UseTexture(depthTarget, AccessFlags.Read);
                builder.SetRenderAttachment(pixelizationMap, 0, AccessFlags.Write);
                builder.AllowGlobalStateModification(true);
                builder.SetRenderFunc(static (PixelMapPassData data, RasterGraphContext context) =>
                {
                    CoreUtils.SetKeyword(data.Material, "ORTHO_PROJECTION", data.Orthographic);
                    CoreUtils.SetKeyword(data.Material, "OVERLAY_CAMERA", data.OverlayCamera);
                    context.cmd.SetGlobalTexture("_MainTex", data.Metadata);
                    context.cmd.SetGlobalTexture("_SourceDepthTexture", data.MetadataDepth);
                    Blitter.BlitTexture(context.cmd, data.Metadata, new Vector4(1f, 1f, 0f, 0f), data.Material, 0);
                });
            }

            using (var builder = renderGraph.AddRasterRenderPass<ApplyPixelizationPassData>(
                "ProPixelizer Apply Pixelization (Color)", out var passData))
            {
                passData.OriginalScene = originalScene;
                passData.PixelizationMap = pixelizationMap;
                passData.MetadataDepth = metadataDepth;
                passData.SceneDepth = depthTarget;
                passData.Material = Materials.ApplyPixelizationMap;
                passData.WriteDepth = false;
                passData.DepthOutputKeyword = ApplyPixelMapDepthOutputKeyword;

                builder.UseTexture(originalScene, AccessFlags.Read);
                builder.UseTexture(pixelizationMap, AccessFlags.Read);
                builder.UseTexture(metadataDepth, AccessFlags.Read);
                builder.UseTexture(depthTarget, AccessFlags.Read);
                builder.SetRenderAttachment(pixelatedScene, 0, AccessFlags.Write);
                builder.AllowGlobalStateModification(true);
                builder.SetRenderFunc(static (ApplyPixelizationPassData data, RasterGraphContext context) =>
                {
                    context.cmd.SetKeyword(data.DepthOutputKeyword, data.WriteDepth);
                    context.cmd.SetGlobalTexture("_MainTex", data.OriginalScene);
                    context.cmd.SetGlobalTexture("_PixelizationMap", data.PixelizationMap);
                    context.cmd.SetGlobalTexture("_SourceDepthTexture", data.MetadataDepth);
                    context.cmd.SetGlobalTexture("_SceneDepthTexture", data.SceneDepth);
                    Blitter.BlitTexture(context.cmd, data.OriginalScene, new Vector4(1f, 1f, 0f, 0f), data.Material, 0);
                });
            }

            using (var builder = renderGraph.AddRasterRenderPass<ApplyPixelizationPassData>(
                "ProPixelizer Apply Pixelization (Depth)", out var passData))
            {
                passData.OriginalScene = originalScene;
                passData.PixelizationMap = pixelizationMap;
                passData.MetadataDepth = metadataDepth;
                ...
                builder.UseTexture(metadataDepth, AccessFlags.Read);
                builder.UseTexture(depthTarget, AccessFlags.Read);
                builder.SetRenderAttachmentDepth(pixelatedDepth, AccessFlags.Write);
                builder.AllowGlobalStateModification(true);
                builder.SetRenderFunc(static (ApplyPixelizationPassData data, RasterGraphContext context) =>
                {
                    context.cmd.SetKeyword(data.DepthOutputKeyword, data.WriteDepth);
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyColorPassData>(
                "ProPixelizer Copy Pixelated Scene", out var passData))
            {
                passData.Source = pixelatedScene;
                builder.UseTexture(pixelatedScene, AccessFlags.Read);
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyDepthPassData>(
                "ProPixelizer Copy Pixelated Depth", out var passData))
            {
                passData.Source = pixelatedDepth;
                passData.Material = Materials.CopyDepth;
                builder.UseTexture(pixelatedDepth, AccessFlags.Read);
                ...
                {
                    context.cmd.SetGlobalTexture("_Depth", data.Source);
